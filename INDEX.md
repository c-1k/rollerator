...
| `scripts/load-time.mjs` | Prints `{ loadMs, readyMs, rollMs, reloadMs }` for a real page load, measured in-page with `performance.now()` against `window.__dice.debug()` rather than the `load` event, which fires before the face textures bake. `readyMs` reads the same as `loadMs` today because the load path is fully synchronous (see the script's header) -- it becomes an independent signal once any of that path goes async. `node scripts/load-time.mjs [origin]`; defaults to `http://localhost:$PORT`. Baseline on `feat/die-redesign` @ `5758633`: median `loadMs` 10152ms over three runs, the first same-method measurement -- not directly comparable to earlier manually-measured figures (`.superpowers/sdd/2026-08-24-die-redesign/task-0-report.md`). |
